...
$execute {
    ...
    if (Save::getLayout().size() == 0){
        ...
        if (oldLayout.size() != 0){
            std::vector<DTLabelInfo> convertedLayout{};
            for (const auto& label : oldLayout)
            {
                DTLabelInfo info;

                info.color = label.color;
                info.font = StatsManager::getFont(label.font);
                info.scale = label.fontSize;
                info.text = label.text;
                info.horizontalAlignment = label.alignment;

                convertedLayout.push_back(info);
            }
