...
void SpecialKey::setUpdateFunction(const UpdateTask& task){
...
void SpecialKey::updateContent(){
    ...
    if (!updateOngoing){
        updateOngoing = true;
        if (this->updateStartedCallback != NULL)
            this->updateStartedCallback(this);
    }
...
void SpecialKey::setUpdateCompletedCallback(const std::function<void(SpecialKey*)>& callback){
    this->updateCompletedCallback = callback;
}

void SpecialKey::setUpdateStartedCallback(const std::function<void(SpecialKey*)>& callback){
...
void SpecialKey::onUpdateCompleted(UpdateTask::Event* e){
    if (auto* val = e->getValue()){
        ...
        if (this->updateCompletedCallback != NULL)
            this->updateCompletedCallback(this);
